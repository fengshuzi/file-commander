...
- ✅ **查找失效文件链接**：支持 wiki link 和标准 Markdown 链接，兼容最简路径与相对路径
- ✅ **一键修正失效文件链接**：扫描后自动更新所有能推荐出新路径的失效链接
- ✅ **一键修复失效图片链接**：扫描后自动把失效图片链接更新为推荐的新路径（wiki/Markdown 两种语法，保留 `|尺寸` 参数）
- ✅ **查找未引用图片**：找出 assets 等配置文件夹下未被任何笔记引用的图片
- ✅ **查找重复笔记**：识别 iCloud 冲突命名（`a.md` / `a 1.md` / `a 2.md`）与内容完全相同的笔记
- ✅ 查找无标签笔记
- ✅ 查找孤立笔记（无入链和出链）
- ✅ 查找空文件
- ✅ **重命名笔记并更新 Markdown 链接**：右键重命名单个笔记，或批量查找替换重命名，自动同步全库 `[文本](路径.md)` 链接
- ✅ 显示选中文件数量统计
- ✅ 右键菜单快速操作
- ✅ 点击文件名快速打开
- ✅ 文件列表按路径排序
...
打开 Obsidian 设置 → 第三方插件 → 浏览，搜索 **File Commander** 或 **fengshuzi** 即可安装。
...
4. 使用工具栏按钮进行批量操作：
   ...
   - **查找失效文件链接**: 查找 wiki link 或标准 Markdown 链接中目标文件已不存在的笔记，支持最简路径与相对路径，并给出修正建议
   - **一键修正失效文件链接**: 扫描后直接自动修正所有能推荐出新路径的失效文件链接
   - **修复失效图片**: 扫描失效图片链接并按完整文件名推荐新路径，预览确认后一键修正（保留 `![[img.png|300]]` 的尺寸参数）
   - **查找未引用图片**: 找出「图片文件夹」配置（如 assets）下未被任何笔记引用的图片，便于清理
   - **重复笔记**: 查找 iCloud 冲突命名（`a 1.md`）和内容完全相同的重复笔记
   - **查找无标签笔记**: 查找没有任何标签的笔记
   - **查找孤立笔记**: 查找既没有入链也没有出链的笔记
   - **查找空文件**: 查找空白或只有 frontmatter 的文件
   - **Markdown 格式检测**: 检查任务列表、标题、列表缩进、未闭合代码块，以及没有配置 language 的围栏代码块
   - **按标签筛选**: 按一个或多个标签筛选文件
   - **按文件夹筛选**: 只显示指定文件夹中的文件
   - **刷新**: 重新加载文件列表
...
检查笔记中指向其他文件（非图片）的链接是否失效，支持 wiki link 和标准 Markdown 链接，并给出修正建议。

**支持的链接格式：**
- Wiki link：`[[工作/供应商预研任务分解-模板|供应商预研任务分解-模板]]`
- Markdown link：`[供应商预研任务分解-模板](工作/供应商预研任务分解-模板.md)`

**路径兼容性：**
- **最简路径**：如 `[[供应商预研任务分解-模板]]` 或 `[文本](供应商预研任务分解-模板.md)`，插件会按文件名全局查找匹配文件
- **相对路径**：如 `[文本](../工作/供应商预研任务分解-模板.md)`，插件会相对于当前笔记解析
- **URL 编码路径**：如 `%20` 等编码字符不会被误判为失效

**使用步骤：**

1. 点击「查找失效文件链接」按钮
2. 插件会扫描整个库中所有 Markdown 文件
3. 文件列表会显示包含失效链接的笔记
4. 弹出检查报告，列出每个失效链接和自动推荐的新路径
5. 点击「使用建议路径」逐个修正，或点击「批量应用」一键修正所有可推荐的链接
6. 如果自动推荐不准确，可点击「手动指定」输入正确路径

**一键自动修正：**
- 点击工具栏「一键修正失效文件链接」，插件会先扫描全库
- 弹出预览确认窗口，列出所有可自动推荐新路径的失效链接及建议路径
- 点击「确认修正」后才会执行替换
- 无法自动推荐的路径不会出现在预览中，可在修正后使用「查找失效文件链接」查看并手动处理
- 修正完成后会提示成功/失败数量，并自动刷新文件列表

**说明：** 该功能只检查内部文件链接，外部链接（http/https）会被忽略；图片链接已由「查找失效图片」处理，不会重复检查。

### 查找未引用图片

在「图片文件夹」设置（如 `assets`）下，找出**没有被任何笔记引用**的图片文件，方便清理无用附件。

1. 在设置中配置好「图片文件夹」（多个用逗号分隔，如 `assets,attachments`）
2. 点击「查找未引用图片」按钮
3. 列表会显示这些文件夹下所有未被 `![[...]]` 或 `[[...]]` 引用的图片
4. 可对结果进行删除、移动等批量操作
...
当图片被移动或重命名导致笔记中的图片链接失效时，可一键修复：

1. 点击「查找与筛选 ▾」→「修复失效图片」
2. 插件扫描全库，弹出预览窗口，列出每个失效图片链接及按完整文件名推荐的新路径
3. 确认后自动替换；`![[img.png|300]]` 中的尺寸参数会保留
4. 未匹配到唯一同名图片的链接不会改动
...
- **iCloud 冲突命名**：同目录下 `a.md` 与 `a 1.md`、`a 2.md` 这类带空格数字后缀的文件
- **内容完全相同**：正文（屏蔽代码块、忽略空白差异）一致的笔记

点击「查找与筛选 ▾」→「重复笔记」，结果会列在文件列表中，可配合批量删除/移动清理。

### 重命名笔记并更新链接

- **单个重命名**：文件列表右键 →「重命名」，输入新名称后自动更新其他笔记中指向它的 `[文本](路径.md)` Markdown 链接（wiki 链接 `[[...]]` 由 Obsidian 自动更新）。
- **批量重命名**：勾选多个文件 →「批量操作 ▾」→「批量重命名」，对文件名做查找替换（支持正则），预览确认后执行并同步更新链接。
...
- 查找失效文件链接时，支持 wiki link、标准 Markdown 链接、最简路径与相对路径
